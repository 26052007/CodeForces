...
 
int miniop(string n, string chance){
    int operations = 0;
    int checker = chance.size()-1;
 
    for(int i = n.size()-1; i >= 0; i--){
        if(n[i] == chance[checker]){
            checker--;
            if(checker < 0){
                break;
            }
...
 
    c+= fill[4];
    c+= fill[3];
    fill[1] = max(0LL, fill[1]-fill[3]);
    if(fill[2]%2 == 0) c+=(fill[2]/2);
    else {
        c+=((fill[2]+1)/2);
        fill[1] = max(0LL, fill[1]-2);
    }
    if(fill[1]%4 == 0) c+= fill[1]/4;
    else{
        c+= (fill[1]/4)+1;
    }
 
